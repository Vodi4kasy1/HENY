local parent = nil;
local objects = {
	["Instance0"] = Instance.new("ScreenGui"); -- HenyСпайс
...
do -- Set properties
	...
	objects["Instance0"]["Name"] = "HenyСпайс";
	...
	objects["Instance4"]["Text"] = "Heny|Спайс | v0.1";
	...
	objects["Instance63"]["Text"] = "Хуета ебаная; Heny Спайс Выдает базу хуйни. не пипидастрические яйца";
	...
	task.spawn(function() -- Instance1
		...
		if not game:GetService("UserInputService").TouchEnabled and game:GetService("UserInputService").KeyboardEnabled and (getfenv().hookmetamethod and getfenv().getnamecallmethod or getfenv().getcallbackvalue or getfenv().hookfunction) then
			...
			local modes = {
				...
				},
                {
					...
					function()
						notification("HenyСпайс вьебал", "Mode")
					end
                }
			}
		...
		if getfenv().hookmetamethod and getfenv().getnamecallmethod and hf then
			if getfenv().getnilinstances and getfenv().getinstances and getfenv().firesignal and getfenv().getcallbackvalue then
				task.spawn(notification, "That executor fully supports HenyСпайс (and simple spy)", "Supported")
			else
				task.spawn(notification, "That executor not fully supports HenyСпайс (but possibly supports simple spy)", "Supported")
			end
		else
			task.spawn(notification, "That does not fully support HenyСпайс (simple spy probably wont work)", "Unsupported")
		end
